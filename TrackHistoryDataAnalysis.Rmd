...
top10songs <- raw_music_pull_dataframe %>% 
  group_by(title) %>% 
  summarize(
  across(where(is.numeric), mean)) %>% 
  slice_max(song_play_count, n = 10, with_ties = FALSE)

p_top10songs_play_count <- ggplot(data = top10songs, aes(reorder(title, song_play_count), y = song_play_count, fill = title, label = song_play_count))+
  geom_col(show.legend = FALSE)+
  geom_text(aes(label = song_play_count))+
  labs(x = NULL, y = NULL) +
  ggtitle(paste("Your Top Played Songs by Number of Plays"))+
  coord_flip()
p_top10songs_play_count
